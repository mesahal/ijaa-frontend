...
const EventMedia = ({ 
  ...
  const loadMedia = async () => {
    ...
    try {
      const response = await axiosInstance.get(`/events/media/event/${eventId}`, {
        headers: {
  ...
  const handleUpload = async (e) => {
    ...
    try {
      ...
      const response = await axiosInstance.post('/events/media', formData, {
        headers: {
      ...
      setMedia(prev => [response.data.data, ...prev]);
      setShowUploadModal(false);
      setSelectedFile(null);
      setUploadForm({ caption: '', type: MEDIA_TYPES.IMAGE });
      onMediaAdded?.(response.data.data);
    } catch (err) {
      setError(err.message);
    } finally {
      setUploading(false);
  ...
  const handleUpdateCaption = async (mediaId) => {
    ...
    try {
      const response = await axiosInstance.put(`/events/media/${mediaId}/caption`, {
      ...
      setMedia(prev => prev.map(item => 
        item.id === mediaId 
          ? { ...item, caption: response.data.data.caption }
          : item
      ));

      setEditingMedia(null);
      setEditCaption('');
      onMediaUpdated?.(response.data.data);
    } catch (err) {
      setError(err.message);
    } finally {
      setLoading(false);
  ...
  const handleDeleteMedia = async (mediaId) => {
    ...
    try {
      await axiosInstance.delete(`/events/media/${mediaId}`, {
        headers: {
          'X-USER_ID': btoa(JSON.stringify({
            username: user?.username,
            userId: user?.id
  ...
  const handleToggleLike = async (mediaId) => {
    try {
      const response = await axiosInstance.post(`/events/media/${mediaId}/like`, {}, {
        headers: {
          'X-USER_ID': btoa(JSON.stringify({
            username: user?.username,
            userId: user?.id
