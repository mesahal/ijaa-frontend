      ...
      try {
        ...
        if (tokenParts.length === 3) {
          ...
          if (timeUntilExpiry < 60) {
            console.warn('⚠️ [AxiosInterceptor] Token expires in less than 1 minute:', {
        ...
        console.warn('⚠️ [AxiosInterceptor] Could not parse token for expiry check:', error.message);
      }
    } else {
  ...
  async (error) => {
    ...
    if (error.response?.status === 401 && !originalRequest._retry) {
      ...
      try {
        ...
        console.log('🔄 [AxiosInterceptor] Retrying original request with new token');
        ...
        if (!isOnLoginPage) {
          ...
          console.log('ℹ️ [AxiosInterceptor] Already on login page - not redirecting');
        }
