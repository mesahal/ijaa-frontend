...
export const featureFlagApi = {
  ...
  checkFeatureFlag: async (featureName) => {
    try {
      ...
      // Use axiosInstance which automatically handles token attachment and refresh
      const response = await axiosInstance.get(
        `/admin/feature-flags/${encodedFeatureName}/enabled`
      );
    ...
    try {
      const promises = featureNames.map(name => 
        featureFlagApi.checkFeatureFlag(name).catch(err => {
          return null; // Return null for failed flags so they're excluded from results
        })
      );
      ...
      results.forEach(result => {
        if (result && result.name) {
          status[result.name] = result.enabled;
        }
        // Note: null results (failed flags) are excluded from the status object
      });
  ...
  checkFeatureFlagWithFallback: async (featureName, fallbackValue = false) => {
    try {
      ...
      return response?.enabled ?? fallbackValue;
    } catch (error) {
